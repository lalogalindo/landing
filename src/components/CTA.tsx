...
export const CTA = () => {
  ...
  return (
    <section id="cta" className="relative overflow-hidden bg-white px-4 py-24 sm:px-6 sm:py-28 lg:px-8">
      ...
      <div className="relative mx-auto max-w-6xl overflow-hidden rounded-[2.5rem] bg-brand-950 p-8 shadow-[0_35px_120px_rgba(16,42,67,0.28)] sm:p-12 lg:p-16">
        ...
        <div className="relative z-10 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <span className="inline-flex rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-black uppercase tracking-[0.2em] text-blue-100">
              {t('cta.eyebrow')}
            </span>
            <h2 className="mt-6 text-4xl font-black tracking-[-0.045em] text-white sm:text-5xl lg:text-6xl">
              {t('cta.title')}
            </h2>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-blue-100/80 sm:text-xl">
              {t('cta.subtitle')}
            </p>
            <button
              onClick={() => {
              ...
              className="group mt-9 inline-flex w-full items-center justify-center rounded-2xl bg-white px-8 py-4 text-lg font-black text-brand-900 shadow-2xl shadow-blue-950/30 transition-all duration-300 hover:-translate-y-1 hover:bg-blue-50 sm:w-auto"
            >
              {t('cta.button')}
            ...
            {highlights.map((item) => (
              <div key={item.label} className="group rounded-3xl border border-white/10 bg-white/[0.08] p-5 backdrop-blur transition-all duration-300 hover:bg-white/[0.13]">
                <div className="flex items-center gap-4">
                  <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-500 text-white shadow-lg shadow-blue-500/25 transition-transform group-hover:scale-110">

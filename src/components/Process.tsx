...
export const Process = () => {
  ...
  return (
    <section id="process" className="relative overflow-hidden bg-brand-950 py-24 text-white sm:py-28">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(37,99,235,0.32),transparent_32%),radial-gradient(circle_at_80%_0%,rgba(56,189,248,0.18),transparent_28%)]" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-start">
          <div className="lg:sticky lg:top-28">
            <span className="text-sm font-black uppercase tracking-[0.26em] text-blue-300">Proceso</span>
          ...
          <div className="relative grid gap-5 md:grid-cols-2">
            <div className="absolute left-1/2 top-10 hidden h-[calc(100%-5rem)] w-px -translate-x-1/2 bg-gradient-to-b from-transparent via-blue-300/40 to-transparent lg:block" />
            {steps.map((step, index) => (
              ...
              >
                <div className="absolute -right-12 -top-12 h-40 w-40 rounded-full bg-blue-400/20 blur-2xl transition-opacity group-hover:opacity-100" />
                <div className="relative z-10">
                  <div className="mb-8 flex items-center justify-between">
                    <span className="text-6xl font-black tracking-[-0.08em] text-white/10">{step.label}</span>
                    <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-brand-accent shadow-lg shadow-blue-900/30">
                      <FontAwesomeIcon icon={step.icon} />
                    </span>
                  </div>
                  <h3 className="text-2xl font-black tracking-[-0.02em]">{t(`process.${step.id}.title`)}</h3>
                  <p className="mt-3 text-base leading-7 text-blue-100/75">{t(`process.${step.id}.desc`)}</p>
                </div>
              </motion.article>
            ))}
          </div>
